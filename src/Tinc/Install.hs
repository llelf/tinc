...
cabalInstallPlan gitCache gitDependencies = withSystemTempDirectory "tinc" $ \dir -> do
  ...
  withCurrentDirectory dir $ do
    ...
    map addGitRevision . parseInstallPlan <$> readProcess "cabal" command ""
  where
  ...
  where
    populate sandbox cachedPackages = do
      withCurrentDirectory sandbox $ do
        packageDb <- initSandbox
          [revisionToPath gitCache name revision | (Package name (Version _ (Just revision))) <- installPlan]
          cachedPackages
        liftIO $ writeGitRevisions packageDb
    ...
    writeGitRevisions packageDb = do
      encodeFile (path packageDb </> "git-revisions.yaml")
        [GitRevision name revision | Package name (Version _ (Just revision)) <- installPlan]
